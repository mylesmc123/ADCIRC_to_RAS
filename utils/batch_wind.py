...
def make_wind(input_file, output_file, map_coords, interval=50, fps=21):
    # open netcdf file in to xarray dataset
    ds = xr.open_dataset(input_file, chunks={"node": 1000})
    ...
    ani = animation.FuncAnimation(fig, update_quiver, fargs=(q, ax, ds), frames=range(0, len(ds["time"])), interval=50)
    ani.save(output_file, writer='ffmpeg', fps=21)

def update_quiver(num, q, ax, ds):
...
if __name__ == "__main__":
    input_file = "/mnt/v/projects/p00832_ocd_2023_latz_hr/01_processing/ADCIRC2RAS/Laura/ras_wind_laura_refTime_20200731_0000.nc"
    output_file = 'wind_laura_batch_test.mp4'
    map_coords={"llcrnrlat":28,
            "urcrnrlat":33.5,
            "llcrnrlon":-95.5,
            "urcrnrlon":-87}
    
    make_wind(input_file, output_file, map_coords)
# # %%
